...
typedef struct {
    int estatDistort;         // 0 -> no s'ha enviat el fitxer; 1 -> s'ha enviat el fitxer
    int n_bytes;          // Bytes escrits (estatDistort = 0); bytes llegits (estatDistort = 1) -> del fitxer
    int fileSize;    // Si estatDistort = 1, s'estava enviant el fitxer distorsionat
    int factor;
    char *fileName;
    int id;
} InfoStatus;
