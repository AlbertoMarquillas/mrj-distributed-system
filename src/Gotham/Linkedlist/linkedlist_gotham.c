...
Element LINKEDLIST_get (LinkedList * list) {
    ...
    // Inicializar element a un valor seguro
    memset(&element, 0, sizeof(Element));

    // Verificar si la lista es NULL o vacía
    if (list == NULL || LINKEDLIST_isAtEnd(*list) || list->previous == NULL || list->previous->next == NULL) {
        list->error = LIST_ERROR_END;
        return element; // Devuelve un elemento vacío para evitar valores no inicializados
    }

    // Si todo es válido, obtenemos el elemento
