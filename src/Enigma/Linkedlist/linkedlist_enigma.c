...
LinkedList *LINKEDLIST_create() {
    ...
    if (list != NULL) {
        ...
        if (list->head != NULL) {
            ...
            free(list);
            list = NULL;  // Devuelve NULL si no se puede asignar memoria para el nodo.
        }
    }
    return list;
}
...
void 	LINKEDLIST_remove (LinkedList * list) {
	...
	if (LINKEDLIST_isAtEnd (*list)) {
    	...
    	list->previous = list->head; // El PREVIOUS debe apuntar al nodo fantasma
	} else {
...
void LINKEDLIST_updateFleckIDs(LinkedList *list) {
    // Mover el POV al inicio de la lista
    LINKEDLIST_goToHead(list);

    int fleck_id = 0; // Inicializar el ID
    // Iterar mientras no estemos al final de la lista
    while (!LINKEDLIST_isAtEnd(*list)) {
        // Obtener el elemento actual
        Element element = LINKEDLIST_get(list);
        
		element.fleck_id = fleck_id;
		LINKEDLIST_remove(list);          // Elimina el nodo actual
		LINKEDLIST_add(list, element);   // Añade el elemento actualizado

        // Avanzar al siguiente nodo en la lista
        LINKEDLIST_next(list);

        // Incrementar el fleck_id
        fleck_id++;
    }
}
...
int LINKEDLIST_getSize(LinkedList *list) {
    int size = 0;
	LINKEDLIST_goToHead(list); // Mover el POV al inicio de la lista
    Node *current = list->head->next; // Saltar el nodo fantasma
    while (current != NULL) {
        size++;
...
void LINKEDLIST_previous(LinkedList *list) {
    // No se puede mover hacia atrás si el POV ya está en el nodo fantasma
    if (list->previous == list->head) {
        list->error = LIST_ERROR_END; // Error: ya en el inicio
    } else {
        // Recorrer desde el nodo fantasma hasta encontrar el nodo anterior
        Node *current = list->head;
        while (current->next != list->previous) {
            current = current->next;
        }
        // Actualizar el puntero previous al nodo anterior
        list->previous = current;
        list->error = LIST_NO_ERROR; // No hubo errores
    }
}


void LINKEDLIST_update(LinkedList *list, Element new_element) {
    // Verifica que la lista sea válida
    if (list == NULL || list->head == NULL) {
        list->error = LIST_ERROR_EMPTY;
        return;
    }

    // Mueve el punto de vista al inicio de la lista
    LINKEDLIST_goToHead(list);

    // Recorre la lista para encontrar el nodo con el mismo `fleck_id`
    while (!LINKEDLIST_isAtEnd(*list)) {
        Element current_element = LINKEDLIST_get(list);

        if (current_element.fleck_id == new_element.fleck_id) {
            // Eliminar y volver a añadir el nodo con el nuevo elemento
            LINKEDLIST_remove(list);
            LINKEDLIST_add(list, new_element);
            return; // Termina tras la actualización
        }

        LINKEDLIST_next(list);
    }

    // Si no se encuentra el nodo, establece un error
    list->error = LIST_ERROR_END;
}
