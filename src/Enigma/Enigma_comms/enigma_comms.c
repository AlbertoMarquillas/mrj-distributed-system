/***********************************************
*
* @Proposit: Fitxer encarregada de les comunicacions de Enigma.
    ...
    while(disconnect == 0) {
        ...
        switch(type){
            case CONNEXIO_FLECK_WORKER:
                ...
                free(pantalla);
                cuaMsg = EnigmaCommsFuncts_actualitzaPunterLinked(info_thread->linked_cua, fleck_id);
                
                if (cuaMsg.estatDistort == 1) {
                    printF("Reenviant fitxer\n");
                    if(EnigmaCommsFuncts_enviarFitxer(fd, info_thread, cuaMsg, fleck_id, contingutFleck)==0){
            ...
            case DISTORT_DADES_FLECK:
                ...
                if(cuaMsg.n_bytes >= contingutFleck.fileSize){
                    if(utils_comparaMD5SUM(fd_file, contingutFleck.md5sum, contingutFleck.path) == 0){
                        ...
                        int newSizeDist = EnigmaCommsFuncts_distorsionarFitxer(extensio, contingutFleck.factor, contingutFleck.path);
                        memset(&contingutFleck.md5sum, '\0', 32);
