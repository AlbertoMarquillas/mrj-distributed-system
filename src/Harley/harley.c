...
#include "Harley_comms/harley_comms.h"

int fd_fleck;
int fd_gotham;
HarleyConfig harley_config_file;
CuaContHarley cont_harley;
...
void gestio_cua_contHarley(int estatConnexio) {
    cont_harley.mtype = 1;
    int msgid_contHarley = HarleyFunct_accedeixCua(1);

    while(msgrcv(msgid_contHarley, &cont_harley, sizeof(CuaContHarley) - sizeof(long), 1, IPC_NOWAIT) >= 0) {
    }

    if(estatConnexio == 0) cont_harley.n_Harley++;
    else cont_harley.n_Harley--;

    if ((msgsnd(msgid_contHarley, &cont_harley, sizeof(CuaContHarley) - sizeof(long), 0) == -1) || (cont_harley.mtype <= 0)) {
        perror("Error enviant missatge a la cua\n");
    }
}

void destroy_cues() {
    int msgid_harley = HarleyFunct_accedeixCua(0);   
    HarleyFunct_eliminaCua(msgid_harley);
    int msgid_fleck = HarleyFunct_accedeixCua(1);
    HarleyFunct_eliminaCua(msgid_fleck);
}

void HarleyFunct_signalHandler(int sig) {
    switch (sig) {
        case SIGINT:
            char trama[MAX_PATH];
            memset(&trama, '\0', MAX_PATH);
            trames_createTramaLogout(trama, harley_config_file.type);
            ...
            gestio_cua_contHarley(1);
            if (cont_harley.n_Harley == 0) {
                destroy_cues();
                eliminarFitxersMediaFiles();
            }

            int msgid_infoFleck = HarleyFunct_accedeixCua(0);
...
void init_recursos() {
    signal(SIGINT, HarleyFunct_signalHandler);
    signal(SIGUSR1, HarleyFunct_signalHandler);
    ...
    int fd_cua_InfoHarley = open(KEY_CONT_HARLEY, O_CREAT | O_WRONLY, 0666);
    if (fd_cua_InfoHarley == -1) {
        perror("Error creant fitxer clau ContHarley");
        exit(1);
    }
    close(fd_cua_InfoHarley);
}


void addThread(CuaInfoFleck msg, HarleyConfig harley_config_file, int fd_fleck) {
    num_threads++;
    ...
    new_fleck_thread[num_threads - 1].harley_config_file = harley_config_file;
    ...
    thread_data->folderName = harley_config_file.folder_name;
    
    // Crear el hilo con su copia independiente
    if (pthread_create(&thread_fleck[num_threads - 1], NULL, HarleyComms_threadFleck, (void *)thread_data) != 0) {
...
int main(int argc, char *argv[]) {
    ...
    if (HarleyFunct_readConfigHarley(argv[1], &harley_config_file) == -1) {
        write(2, ERROR_FILE, strlen(ERROR_FILE));
        exit(1);
    }

    fd_gotham = HarleyFunct_ConnectToGotham(harley_config_file, linked_cua_global);
    if (fd_gotham < 0) {
        perror("Error connectant amb Gotham");
        exit(1);
    }

    gestio_cua_contHarley(0);

    GothamThread *info_gotham_thread = malloc(sizeof(GothamThread));
    info_gotham_thread->fd = fd_gotham;
    if (pthread_create(&gotham_thread, NULL, HarleyComms_heartbeatThreadGotham, (void *)info_gotham_thread) != 0) {
        perror("Error creant thread de heartbeat");
        exit(1);
    }

    if (HarleyFunct_esperaWorkerPrincipal(fd_gotham) == -1) {
        perror("Error esperant worker principal");
        exit(1);
    }

    int fd = Connexions_openServer(atoi(harley_config_file.fleck_port), harley_config_file.fleck_ip);
    if (fd < 0) {
    ...
    while (1) {
        ...
        addThread(msg, harley_config_file, fd_fleck);
    }
